...
class TreeEnsemble:
    ...
    def __init__(self, model, data=None, data_missing=None, model_output=None):
        ...
        # Added Line 992 to 1008 for AdaBoostClassifier and AdaBoostRegressor (Yixian)
            ...
            self.tree_output = "probability"
        elif safe_isinstance(model, ("sklearn.ensemble._weight_boosting.AdaBoostRegressor", "sklearn.ensemble.AdaBoostRegressor")):
            assert hasattr(model, "estimators_"), "Model has no `estimators_`! Have you called `model.fit`?"
            self.internal_dtype = model.estimators_[0].tree_.value.dtype.type
            self.input_dtype = np.float32
            scaling = 1.0 / len(model.estimators_) # output is average of trees
